...
You should see that multiple `tf2_ros/static_transform_publisher` nodes are launced when running
...
The coloring of the axes are ordered as R, G then B, corresponding to the first, second and third element in the position vector. For NED this will then be:  
* Red: North  
* Green: East  
* Blue: Down  
...
However, they are all overlapping. This is because the coordinate transforms as defined in `static_frames.launch.py` are all zeros. The actual values for these depends on the physical configuration of the vessel we want to represent. For this tutorial we use a simplified example from our AUV Orca.
...
Fix the arguments to the transform in `static_frames.launch.py` and visualize the correct frames in foxglove (Hint: the rotations can be solved first be a rotation around X, followed by a rotation around the Z axis)

Your result should look like this:  
![base_link_camera_transform](./img/base_link_camera.png)  

Althought the base_link frame uses the NED convention, it looks here like the z-axis is pointing upwards. This is just because we don't have anything else to compare it against.
...
We will introduce a short example to highlight to help understand the different conventions.

A prerequisite for this task is the install the xacro package for ros2 humble
...
For this example only `person_enu_link`, `base_link` and `camera` is needed.
...
With `base_link` still selected as the `display frame` you 3D panel should look like this:
...
The file `src/dynamic_transform_publisher_ros.cpp` contains boiler plate code for a dynamic transform, and your goal is to make the *base_link* frame rotate about *odom* in the xy plane, with the forward direction of the *base_link* frame (red) pointing in the direction of movement.
...
As a final tough we can edit the transform for the person object. Go into the `person.launch.py` file and change the parent of the transform to odom. Select `person_enu_link` as the display frame to use it as a Z-up visualization frame.
